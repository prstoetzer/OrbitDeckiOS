...
- **Track & predict** — live look angles, range/range-rate, sub-point and footprint; next-pass predictions with elevation profiles; ground-track map; all-sky radar; 3D globe and polar sky plot with direction-of-travel and AOS/LOS arrows; a daily schedule of every favorite's passes ordered by AOS, day by day.
- **Rig control (CAT)** — Doppler-tune your transceiver from OrbitDeck over a BLE serial adapter (Icom CI-V, Yaesu, Kenwood — including the TS-2000/TS-790 in satellite mode) or an Icom network (RS-BA1) connection over Wi-Fi; single full-duplex radio, a single uplink/downlink radio, or two radios; narrow FM (FM-N) on FM birds and the data sub-mode (USB-D/LSB-D) for FT4. Configured in Settings, controlled on Home. (Bluetooth Classic SPP adapters are not supported on iOS.) A known-good DIY BLE&nbsp;↔&nbsp;CI-V bridge is [OrbitLinkS3](https://github.com/prstoetzer/OrbitLinkS3) on an M5Stack M5StickS3.
- **Rotator control** — steer an az/el antenna rotator to follow the pass over a BLE serial adapter (GS-232, Easycomm I/II/III, SPID Rot2Prog, SAEBRTrack) or the network (rotctld/Hamlib over TCP, PstRotator over UDP, OZ9AAR URC over TCP/JSON); pre-position before AOS, a slew lead to aim ahead of the bird, park on LOS, flip mode, 0–360 / ±180 / 0–450° azimuth ranges, alignment offsets, magnetic correction, deadband and minimum-elevation gating. Configured in Settings, controlled on Home.
- **Logging & uploads** — on-device satellite QSO log with a Home quick-log card and a dedicated Log screen; **LoTW** upload with the `.tq8` signed on device from your imported callsign certificate (.p12); **Cloudlog/Wavelog** upload; ADIF import/export; automatic LoTW satellite-name normalization.
- **Digital modes & audio** — over a USB audio interface (or an Icom network-audio radio): record a pass, decode **SSTV** (Doppler-aware auto-tune that keeps fast-LEO pictures straight and true-colored, feed-forward from rig control, an image gallery with a post-pass "fix image" / re-decode-from-recording, and export to Photos), and work **full-duplex FT4** on a linear transponder (decode the downlink while transmitting the uplink, with a live Doppler-corrected RX/TX frequency readout for tuning by hand and optional automatic per-satellite transponder calibration from your own decoded signal; PTT over CAT — CI-V/Yaesu/Kenwood/rigctld/Icom-net — or a manual-PTT prompt). FT4/FT8 use the MIT [ft8_lib](https://github.com/kgoba/ft8_lib). FT4 is ~100% duty cycle — limit power.
- **rigctl (Hamlib)** — drive any Hamlib-supported radio via a `rigctld` network connection (a CAT connection type), for full-duplex split or a single leg.
...
- **Activations & status** — upcoming satellite activations from hams.at with a "can I work it?" mutual-window check; post your own activation to hams.at (grid lines/corners supported) with your API key; QRZ XML callsign lookup; AMSAT status board and one-tap attributed status reporting from Home.
